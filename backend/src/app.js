...
// Allow the configured production domain + Vercel/Cloudflare preview deployments
var corsOriginAllowed = (origin) => {
  if (!origin) return true;                                     // server-to-server / curl
  if (origin === CONFIG.FRONTEND_DOMAIN) return true;          // production domain
  if (/\.vercel\.app$/.test(origin)) return true;              // Vercel preview URLs
  if (/\.pages\.dev$/.test(origin)) return true;               // Cloudflare Pages previews
...
// SPA fallback for all non-API routes (React client-side routing)
app.get(/^(?!\/api).*/, (req, res) => {
  const indexPath = path.join(frontendDist, "index.html");
  res.sendFile(indexPath, (err) => {
    if (err) {
      res.send("⚡️ RestroPRO Backend API is running.");
    }
  });
});
