...
exports.updateGatewayStatusDB = async (paymentGatewayName, status) => {
    ...
    try {
        ...
        await conn.query(`UPDATE payment_gateways SET status = 0`);
...
exports.activatePaymentGatewayDB = async () => {
    const conn = await getMySqlPromiseConnection();
    try {
        const [rows] = await conn.query('SELECT id, gateway_name, status, credentials, created_at FROM payment_gateways WHERE status = 1');
